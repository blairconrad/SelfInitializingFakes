namespace SelfInitializingFakes.Tests.Conventions
{
    ...
    public class Naming
    {
        ...
        {
            typeof(SelfInitializingFake<>).GetTypeInfo().Assembly.GetTypes()
                .Where(t => typeof(IRecordedCallRepository).IsAssignableFrom(t) && !t.Name.EndsWith("RecordedCallRepository"))
                .Should().BeEmpty();
        }
        ...
        {
            typeof(SelfInitializingFake<>).GetTypeInfo().Assembly.GetTypes()
                .Where(t => t.Name.EndsWith("RecordedCallRepository") && !typeof(IRecordedCallRepository).IsAssignableFrom(t))
                .Should().BeEmpty();
        }
    }
}
